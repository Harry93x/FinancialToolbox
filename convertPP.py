...
def main(argv):
  ...
  interests = [] #= np.zeros(inWorksheet.nrows, dtype=[('date', 'U10'), ('amount', 'f4')])
  concatInterests = []
  ...
  while iRow < inWorksheet.nrows:
    if inWorksheet.cell(iRow, iColTransactionType).value == transactionType:
      if inWorksheet.cell(iRow, iColDescription).value == descriptionType:
        ...
        #interests = interests.append([inWorksheet.cell(iRow,iColDate).value, inWorksheet.cell(iRow,iColAmount).value])
        
        interests.append([lastDate, interest])
    iRow += 1
        #interests[iRow] = (inWorksheet.cell(iRow,iColDate).value, inWorksheet.cell(iRow,iColAmount).value)

  #for iRow in xrange(0, len(interests(0)):
  #  lastDate 
